...
<h4>Byte Lengths and Word Spans</h4>


This BIP uses the `length(...)` and `wordspan(...)`
convention from <a href="/440" target="_blank">BIP440</a>: `length(X)` is the
...
|OP_SUBSTR|127|<nowiki>[A BEGIN LEN]</nowiki>|Extract bytes BEGIN through BEGIN+LEN of A||(wordspan(LEN) + wordspan(BEGIN)) * 2 + MIN(Value of LEN, MAX(length(A) - Value of BEGIN, 0)) * 3|LENGTHCONV + COPYING|
|OP_LEFT|128|<nowiki>[A OFFSET]</nowiki>|Extract the left OFFSET bytes of A||wordspan(OFFSET) * 2|LENGTHCONV|
|OP_RIGHT|129|<nowiki>[A OFFSET]</nowiki>|Extract the rightmost OFFSET bytes of A||wordspan(OFFSET) * 2 + MIN(Value of OFFSET, length(A)) * 3|LENGTHCONV + COPYING|
...
|Mnemonic|Opcode|Input Stack|Description|Definition|Varops Cost|Varops Reason|
|-|-|-|-|-|-|-|
|OP_INVERT|131|<nowiki>[A]</nowiki>|Bitwise invert A||wordspan(A) * 4|OTHER|
|OP_AND|132|<nowiki>[A B]</nowiki>|Binary AND of A and B||(wordspan(A) + wordspan(B)) * 2|OTHER + ZEROING|
|OP_OR|133|<nowiki>[A B]</nowiki>|Binary OR of A and B||MIN(wordspan(A), wordspan(B)) * 4|OTHER|
|OP_XOR|134|<nowiki>[A B]</nowiki>|Binary exclusive-OR of A and B||MIN(wordspan(A), wordspan(B)) * 4|OTHER|
...
|Mnemonic|Opcode|Input Stack|Description|Definition|Varops Cost|Varops Reason|
|-|-|-|-|-|-|-|
|OP_UPSHIFT|152|<nowiki>[A BITS]</nowiki>|Move bits of A right by BITS (numerically increase)||wordspan(BITS) * 2 + (Value of BITS) / 8 * 2 + length(A) * 3.  If BITS % 8 != 0, add wordspan(length(A) + (Value of BITS) / 8) * 4|LENGTHCONV + ZEROING + COPYING. If BITS % 8 != 0, + OTHER over the shifted result length.|
|OP_DOWNSHIFT|153|<nowiki>[A BITS]</nowiki>|Move bits of A left by BITS (numerically decrease)||wordspan(BITS) * 2 + MAX((length(A) - (Value of BITS) / 8), 0) * 3|LENGTHCONV + COPYING|
...
|Mnemonic|Opcode|Input Stack|Description|Definition|Varops Cost|Varops Reason|
|-|-|-|-|-|-|-|
|OP_2MUL|141|<nowiki>[A]</nowiki>|Multiply A by 2||wordspan(A) * 7|OTHER + COPYING|
...
|Mnemonic|Opcode|Input Stack|Description|Definition|Varops Cost|Varops Reason|
|-|-|-|-|-|-|-|
|OP_ADD|147|<nowiki>[A B]</nowiki>|Add A and B||MAX(wordspan(A), wordspan(B)) * 9|ARITH + COPYING|
|OP_1ADD|139|<nowiki>[A]</nowiki>|Add one to A||MAX(wordspan(1), wordspan(A)) * 9|ARITH + COPYING|
|OP_SUB|148|<nowiki>[A B]</nowiki>|Subtract B from A where B is <= A||MAX(wordspan(A), wordspan(B)) * 6|ARITH|
|OP_1SUB|140|<nowiki>[A]</nowiki>|Subtract 1 from (non-zero) A||MAX(wordspan(1), wordspan(A)) * 6|ARITH|
...
|-|-|-|
|OP_CHECKLOCKTIMEVERIFY|wordspan(operand) * 2|LENGTHCONV|
|OP_CHECKSEQUENCEVERIFY|wordspan(operand) * 2|LENGTHCONV|
|OP_CHECKSIGADD|MAX(wordspan(1), wordspan(number operand)) * 9 + 500,000|ARITH + COPYING + SIGCHECK|
...
loops, using `wordspan(n)` from <a href="/440" target="_blank">BIP440</a>, as
...
1.  Compare: cost = wordspan(A) * 2 (COMPARING)


1.  Subtract: cost = wordspan(A) * 6 (ARITH)


1.  for (wordspan(A) - NormalizedLength(B)) in words:
    1.  Multiply word by B -> scratch: cost = NormalizedLength(B) * 6 (ARITH)
    1.  Subtract scratch from A: cost = wordspan(A) * 6 (ARITH)
    1.  Add B into A (no overflow): cost = wordspan(A) * 6 (ARITH)
    1.  Shrink A by 1 word.


1.  OP_MOD: shift A down, trim trailing zeroes: cost = wordspan(A) * 4


1.  OP_DIV: trim trailing zeros: cost = wordspan(A) * 4
