...
The function `ms32_verify_checksum` selects the checksum variant required by codex32 and MUST return true when its argument is the data part as a list of integers representing the characters converted using the bech32 character table from BIP-0173.
The functions `ms32_verify_regular_checksum` and `ms32_verify_long_checksum` verify only their respective checksum variants within their stated periods.
...
For the required human-readable part `ms`, the expansion contributes five values.
...
1.  Choose a bit size from 128, 160, 192, 224, 256, or 512
...
1.  Set the payload to a bech32 encoding of the application-specified secret payload bits; for a master seed, follow "Master seed format".
...
A complete Python implementation is available in the <a href="https://github.com/BenWestgate/python-codex32" target="_blank">python-codex32 repository</a>.
The <a href="https://github.com/BlockstreamResearch/codex32" target="_blank">original project repository</a> contains implementations in Rust and PostScript.
...
<h3>Test vectors 6, 7, and 8</h3>


These unshared codex32-encoded master seeds use the identifier `seed` and cover all supported master seed sizes not shown by Test vectors 1--5.

*  160-bit master seed (hex): `000102030405060708090a0b0c0d0e0f10111213`
    *  codex32 secret: `ms10seedsqqqsyqcyq5rqwzqfpg9scrgwpugpzysn9vaqzzvs20xnl`
*  192-bit master seed (hex): `202122232425262728292a2b2c2d2e2f3031323334353637`
    *  The three discarded bits have the nonzero value `101`.
    *  codex32 secret: `ms10seedsyqsjygeyy5nzw2pf9g4jctfw9ucrzv3nxs6nvdau84gz0632s0xs`
*  224-bit master seed (hex): `404142434445464748494a4b4c4d4e4f505152535455565758595a5b`
    *  The discarded bit has the value `1`.
    *  codex32 secret: `ms10seedsgpq5ys6yg4rywjzfff95cn2wfag9z5jn2324v46ct9d9hrcduqw8c3lccl`
